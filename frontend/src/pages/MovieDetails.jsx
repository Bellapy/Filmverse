  ...
  return (
    <div className="bg-film-black min-h-screen text-white animate-fade-in">
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-40 relative z-10 pb-20">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-1/5 flex-shrink-0">
            <img src={movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}`: 'https://via.placeholder.com/500x750?text=Sem+Imagem'} alt={movie.title} className="rounded-lg shadow-2xl border-4 border-film-gray w-full" />
          ...
          <div className="w-full md:w-4/5 md:pt-16 space-y-8">
      ...
      {similarMovies && similarMovies.length > 0 && (
        <div className="-mt-10">
          <MovieRow title="Títulos Semelhantes" movies={similarMovies} size="sm" />
        </div>
      )}
      {showTrailer && trailer && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={() => setShowTrailer(false)}>
          <div className="relative w-full max-w-4xl animate-fade-in">
            ...
            <div className="aspect-video">
              <iframe src={`https://www.youtube.com/embed/${trailer.key}?autoplay=1`} title={movie.title} className="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
            </div>
          </div>
        </div>
      )}
